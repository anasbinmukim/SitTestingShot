...
<?php 
	...
	//echo '<div>'.html_entity_decode($message->msg_content).'</div>';
	foreach($message_info as $message){
		echo '<div class="reply_text">Message from '.$message->last_name.':</div>';
		echo '<div>'.html_entity_decode($message->msg_content).'</div>';		
	}
 ?>
...
<?php
	foreach($message_reply as $reply){
		echo '<div class="message_reply">';
		echo '<div class="reply_text">Reply from '.$reply->last_name.':</div><div>'.$reply->msg_content.'</div>';
	?>
	...
	<div class="row">
		<div class="col-md-12">
		  <div class="form-group">
			  ...
			  <textarea class="form-control forcs_background" name="msg_content"></textarea></div>		  
		  <div class="reply_btn">
